...
- (void)setDate:(NSDate *)date {
    ...
    NSInteger yearIndex = kCenterCell + [nowComponents year] - [dateComponents year] - 100;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == monthTableView) {
        ...
        NSDateComponents *nowComponents = [[NSCalendar currentCalendar] components:NSCalendarUnitYear fromDate:[NSDate date]];
        cell.label.text = [NSString stringWithFormat:@"%ld", (long)[nowComponents year] - (indexPath.row % 200) + 100];
    }
